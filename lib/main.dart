...
class ClubStats {
  ...
  factory ClubStats.fromJson(Map<String, dynamic> json) => ClubStats(
      json['name'], json['carry'], json['sideError'], json['depthError']);
}
...
class GolfHole {
  ...
  GolfHole(
      ...
      required this.tee,
      required this.green});
  LatLng get center => LatLng((tee.latitude + green.latitude) / 2,
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text("Professional Golf Analytics",
                  ...
                  SizedBox(
                      ...
                      child: ElevatedButton(
                          ...
                          onPressed: _login,
                          child: const Text("ENTER CLUBHOUSE",
                              style: TextStyle(
                                  color: Colors.white,
                                  fontWeight: FontWeight.bold))))
                ],
                      ...
                      child: Column(
                        ...
                        children: const [
                          ...
                          _HelpItem(
                              ...
                              title: "开球修正",
                              desc:
                                  "在第一杆发球时，如果不在默认发球台，点击底部 '🎯 从当前GPS开球' 修正位置。"),
                          _HelpItem(
                              icon: Icons.near_me,
                              title: "GPS 记录落点",
                              desc:
                                  "打完球走到球边，点击红色 '📍 记录落点' 按钮。系统会通过 GPS 计算距离并绘制轨迹。"),
                          _HelpItem(
                          ...
                          _HelpItem(
                              ...
                              title: "风速调整",
                              desc:
                                  "点击顶部风扇图标，设置风向风速。App 会自动计算受风影响后的 'Plays Like' 距离。"),
                          _HelpItem(
                              icon: Icons.flag_circle,
                              title: "记分与GIR",
                              desc:
                                  "打完一洞后点击右上角旗帜，输入推杆数。系统自动计算总分和是否 GIR (标准杆上果岭)。"),
                          _HelpItem(
                              icon: Icons.history,
                              title: "复盘模式",
      ...
      body: ListView(
        children: [
          SwitchListTile(
            ...
            subtitle: Text(_isYards ? "Yards (码)" : "Meters (米)",
...
class _GolfMapScreenState extends State<GolfMapScreen> {
  ...
  Future<void> _saveRoundData() async {
    ...
    prefs.setString(
        'scorecard', jsonEncode(_scorecard.map((e) => e.toJson()).toList()));
    prefs.setString(
        'shots', jsonEncode(_allShotsHistory.map((e) => e.toJson()).toList()));
  ...
  Future<void> _recordLocationAsLandingPoint() async {
    ...
    try {
      ...
      double distFromHoleCenter =
          _calculateDistance(gpsPos, _currentHole.center);
                ...
                actions: [
                  ...
                  ElevatedButton(
                      ...
                      onPressed: () {
                        Navigator.pop(ctx);
                        double dist =
                            _calculateDistance(_currentBallPos, gpsPos);
    ...
    showDialog(
        ...
        builder: (ctx) {
          ...
          return StatefulBuilder(builder: (context, setState) {
            return AlertDialog(
                backgroundColor: Colors.grey[900],
                title: Text(isManual ? "手工落点" : "GPS 落点",
                    style: const TextStyle(color: Colors.white)),
                content: Column(mainAxisSize: MainAxisSize.min, children: [
                  Text(_formatDist(distance),
                      style: const TextStyle(
                          fontSize: 30,
                          color: Colors.greenAccent,
                          fontWeight: FontWeight.bold)),
  ...
  void _finishHoleDialog() {
    ...
    var existingScore = _scorecard.firstWhere(
        ...
        orElse: () => HoleScore(
            holeNumber: 0, par: 0, shotsTaken: 0, putts: 2, penalties: 0));
                          ...
                          if (!_isReplayMode)
                            SizedBox(
                                ...
                                child: ElevatedButton(
                                    ...
                                    child: const Text("FINISH ROUND & ARCHIVE",
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold))))
                        ])))),
          );
